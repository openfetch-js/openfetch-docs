...
5. [الإضافات والواجهة السلسة](./plugins-fluent.md) — مسارات `plugins` و`sugar`، ترتيب التسجيل  
6. [المعترضات والوسيط](./interceptors-middleware.md) — ترتيب التنفيذ، `use()`، وسيط مخصص  
7. [البنية والداخلية](./architecture.md) — رسوم معمارية، مقارنة Axios، مراجعة تصميم، شرح `retry` و`cache`  
8. [إعادة المحاولة والتخزين المؤقت](./retry-cache.md) — `createRetryMiddleware`، `createCacheMiddleware`، TTL / SWR  
9. [الأخطاء والأمان](./errors-security.md) — `OpenFetchError`، الرموز، التسجيل الآمن، `assertSafeHttpUrl`  
