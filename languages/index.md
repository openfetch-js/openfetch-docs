---
layout: home

hero:
  name: openFetch
  text: Documentation
  tagline: Fetch-only HTTP client — interceptors, middleware, retry, memory cache, RSC-friendly.
  actions:
    - theme: brand
      text: English
      link: /
    - theme: alt
      text: العربية
      link: /ar/
    - theme: alt
      text: Español
      link: /es/

features:
  - title: Français
    details: French documentation
    link: /fr/
    linkText: Open
  - title: فارسی
    details: RTL · Persian
    link: /fa/
    linkText: Open
  - title: हिन्दी
    details: Hindi documentation
    link: /hi/
    linkText: Open
  - title: Italiano
    details: Italian documentation
    link: /it/
    linkText: Open
  - title: 日本語
    details: Japanese documentation
    link: /ja/
    linkText: Open
  - title: 한국어
    details: Locale path /kr/ (lang ko)
    link: /kr/
    linkText: Open
  - title: Kurdî
    details: Kurdish documentation
    link: /ku/
    linkText: Open
---

### Package

Install **`@hamdymohamedak/openfetch`** from npm. Docs follow the public API around **v0.2.x**.

Use the **language menu** (globe icon) in the navbar on doc pages to jump to the same page in another locale when a translation exists.

For architecture and request flow, see the package repository **`openFetch/docs/PROJECT_FLOW.md`**.
