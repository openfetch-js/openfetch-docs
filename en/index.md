# openFetch — English (en)

**@hamdymohamedak/openfetch** is a small, dependency-free HTTP client for any JavaScript runtime that provides the standard [`fetch`](https://developer.mozilla.org/en-US/docs/Web/API/Fetch_API) API. It adds instances with defaults, request/response **interceptors**, composable **middleware**, optional **retry** with backoff, **in-memory caching** for GET/HEAD, structured **errors**, and an optional **URL guard** for untrusted URLs — without coupling to React, `window`, or legacy XHR.
...
2. [Configuration](./configuration.md) — full request config, response shape, `unwrapResponse`  
3. [Interceptors & middleware](./interceptors-middleware.md) — execution order, `use()`, custom middleware  
4. [Retry & cache](./retry-cache.md) — `createRetryMiddleware`, `createCacheMiddleware`, TTL / SWR  
5. [Errors & security](./errors-security.md) — `OpenFetchError`, codes, safe logging, `assertSafeHttpUrl`  
