...
## Raw native `Response` (`rawResponse`)

Set **`rawResponse: true`** on a call when you want the **`fetch`** **`Response`** object without openFetch reading the body yet (streaming control, custom parsing, or passing through to storage):

```ts
const res = await api.request<Response>("/v1/export", {
  method: "GET",
  rawResponse: true,
  unwrapResponse: true,
});
...
**`transformResponse`** does **not** run in this mode. **Response interceptors** still run with **`data`** = that **`Response`**.

With the fluent client, **`.raw()`** is the terminal equivalent — see [Plugins & fluent API](./plugins-fluent.md).
