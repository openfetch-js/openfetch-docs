...
`attempts` is an alias for **`maxAttempts`**. See [Plugins & fluent API](./plugins-fluent.md).
...
**Implementation note:** background refresh calls **`dispatch` directly**, not the full client **`use()`** stack. Custom middleware on the instance (logging, auth refresh) does **not** run for that background fetch. Shared behavior should live in **`transformRequest` / defaults** or a documented workaround. See [Architecture & internals](./architecture.md).
